...
# mozart-compile.py - Compiler for creating transposed and mirror-image scores
...
help = """Mozart-compile - Compiler for creating transposed and mirror-image scores
Written by: Benjamin Pritchard / Kundalini Software
www.kundalinisoftware.com
...
NOTE:   please see the Lilypond Scores in mozart-compile-input for example input files; they are slightly not
        exactly in lilypond format!! (This needs documented...)
...
26-Sept-2021    Version 1.1     cleanup; moved to linux platform; better octava handling
"""
...
def tokenize(input):
    ...
    Notes = []
    for token in tokens:
        # 0 will mean we won't know absolute pitch number yet
        note = Note(token, False, 0)
        Notes.append(note)
...
def transpose(text, original_key, original_mode, transposition_str, isTreble):
    fd, path = mkstemp()
    ...
    with open(output_file, 'r') as f:
        ...
        # grab the calculated key that LY figured out
        new_key = f.readline().split(" ")[1]
...
def half_steps_to_pitch_interval_1(key, mode, half_steps):
    ...
    if (mode.upper() == "MAJOR"):
        ...
        if (key == "c"):
            interval_string = "c cis d ees e f fis g aes a bes b"
        if (key == "g"):
            interval_string = "g aes a bes b c' cis' d' ees' e' f' fis'"
        if (key == "d"):
            interval_string = "d ees e f fis g aes a bes b c' cis'"
        if (key == "a"):
            interval_string = "a bes b c' cis' d' ees' e' f' fis' g' aes'"
        if (key == "e"):
            interval_string = "e f fis g aes a bes b c' cis' d' ees'"
        if (key == "b"):
            interval_string = "b c' cis' d' ees' e' f' fis' g' gis' a ais'"
        if (key == "ces"):
            interval_string = "ces c des d ees e f ges g aes a bes"
        if (key == "fis"):
            interval_string = "fis g gis a ais b c' cis' d' dis' e'"
        if (key == "ges"):
            interval_string = "ges g aes a bes b c' des' d' ees' e' f'"
        if (key == "des"):
            interval_string = "des d ees e f ges g aes a bes b c'"
        if (key == "cis"):
            interval_string = "cis d dis e f fis g gis a ais b"
        if (key == "aes"):
            interval_string = "aes a bes b c' des' d' ees' e' f' ges' g' a'"
        if (key == "ees"):
            interval_string = "ees e f ges g aes a bes b c' des' d'"
        if (key == "bes"):
            interval_string = "bes c' des' d' ees' e' f' ges' g' aes' a' b"
        if (key == "f"):
            interval_string = "f ges g aes a bes b c' des' d' ees' e'"

    else:
        # handle all minor keys
        if (key == "a"):
            interval_string = "a bes b c' des' d' ees' e' f' ges' g' aes'"
        if (key == "e"):
            interval_string = "e f fis g gis a bes b c' cis' d' dis'"
        if (key == "b"):
            interval_string = "b c' cis' d' dis' e' f' fis' g' gis' a' ais'"
        if (key == "fis"):
            interval_string = "fis g gis a ais b c' cis' d' dis' e' f'"
        if (key == "cis"):
            interval_string = "cis d dis e f fis g gis a ais b c'"
        if (key == "gis"):
            interval_string = "gis a ais b c' cis' d' dis' e' f' fis' g'"
        if (key == "ees"):
            interval_string = "ees e f ges g aes a bes b c' des' d'"
        if (key == "dis"):
            interval_string = "dis e f fis g gis a ais b c' cis' d'"
        if (key == "bes"):
            interval_string = "bes b c' des' d' ees' e' f' ges' g' aes' a'"
        if (key == "f"):
            interval_string = "f ges g aes a bes b c' des' d' ees' e'"
        if (key == "c"):
            interval_string = "c des d ees e f ges g aes a bes b"
        if (key == "g"):
            interval_string = "g aes a bes b c' des' d' ees' e' f' ges'"
        if (key == "d"):
            interval_string = "d ees e f ges g aes a bes b c' des'"
    ...
    if (mode.upper() == "MAJOR"):
        ...
        if (leftMatch(interval_2, "ges")):
            interval_2 = "fis"             # gflat minor doesn't exist... convert to f# minor
        if (leftMatch(interval_2, "aes")):
            interval_2 = "gis"             # aflat minor doesn't exist... convert to g# minor
        if (leftMatch(interval_2, "des")):
            interval_2 = "cis"             # dflat minor doesn't exist... convert to c# minor
        if (leftMatch(interval_2, "ais")):
            interval_2 = "bes"             # asharp minor doesn't exist... convert to bflat minor
...
if (mode == 0):
    ...
    transposition_str = half_steps_to_pitch_interval_1(
...
if (mode == 1):
    ...
    key_1 = original_key_letter
    # key_2 will have same mode as key_1 always
...
output_template = output_template.replace(
    "{half_step_transposition}", to_half_step_string(half_steps_to_transpose))

output_template = output_template.replace("{key_1}", to_ly_key(
    key_1, original_key_mode))               # major/minor doesn't change
output_template = output_template.replace("{time_1}", time_1)
output_template = output_template.replace(
    "{voice_one_block}", voice_one_block)

output_template = output_template.replace("{key_2}", to_ly_key(
    key_2, original_key_mode))               # major/minor doesn't change
output_template = output_template.replace("{time_2}", time_2)
output_template = output_template.replace(
    "{voice_two_block}", voice_two_block)
